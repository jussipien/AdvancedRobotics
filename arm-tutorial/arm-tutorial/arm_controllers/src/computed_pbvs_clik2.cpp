...
namespace arm_controllers
{
    ...
    // ros::Subscriber sub_aruco_pos = nh.subscribe<geometry_msgs::PoseStamped>("pose", 1, posAruco);
...
class Computed_Torque_Controller_CLIK : public controller_interface::Controller<hardware_interface::EffortJointInterface>
{
  public:
    bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    {
        ...
        sub_aruco_pos_ = nh.subscribe<geometry_msgs::PoseStamped>("pose", 1, &Computed_Torque_Controller_CLIK::posAruco, this);
        ...
        // sub_aruco_pos = nh.subscribe<geometry_msgs::PoseStamped>("pose", 1, &Computed_Torque_Controller_CLIK::posAruco, this);
        // event_ = 0; // subscribe 받기 전: 0
                    // subscribe 받은 후: 1
    ...
    void update(const ros::Time &time, const ros::Duration &period)
    {
        ...
        if (ctr_obj_ == 1)
        {
            ...
            else if (event_ == 1) // command from ros subscriber
            {
                ...
                x_.p(0) = transform_.getOrigin().x();
                x_.p(1) = transform_.getOrigin().y();
                x_.p(2) = transform_.getOrigin().z();
                
                xd_.p(0) = transform2_.getOrigin().x();
                xd_.p(1) = transform2_.getOrigin().y();
                xd_.p(2) = transform2_.getOrigin().z();
